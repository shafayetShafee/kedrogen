...
def move_contents(src_dir: Path, dest_dir: Path, logger: Logger):
    ...
    for item in src_dir.iterdir():
        ...
        try:
            ...
            logger.error(
                f"[red][x] Failed to move [bold]'{item.name}'[/bold]: {e}[/red]"
            )
...
def generate(
    template_path: Annotated[
        str, typer.Argument(help="Path or Git URL of the Cookiecutter template.")
    ],
    verbose: Annotated[
        Optional[bool], typer.Option("--verbose", help="Show detailed output.")
    ] = False,
    quiet: Annotated[
        Optional[bool],
